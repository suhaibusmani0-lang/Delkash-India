...
export default function WhatsAppButton() {
  ...
  const phoneNumber = '919871127869';
  const defaultMessage = encodeURIComponent(
    'Hello Adv. Rahimullah Ansari, I would like to consult regarding Trademark/IPR legal services.'
  );
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center group">
      {/* Tooltip Pill */}
      <div
        className={`mr-3 px-3.5 py-1.5 rounded-full bg-navy-900/95 text-white text-xs font-medium border border-gold-500/30 shadow-xl transition-all duration-300 pointer-events-none hidden sm:flex items-center space-x-1.5 ${
          isHovered ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'
        }`}
      >
        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
        <span>Chat with Adv. Rahimullah</span>
      </div>
